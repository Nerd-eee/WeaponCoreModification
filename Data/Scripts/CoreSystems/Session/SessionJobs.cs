                ...
                if (TopEntityToInfoMap.TryGetValue(grid, out topMap))
                {
                    ...
                    for (int j = 0; j < allFat.Count; j++)
                    {
                        ...
                        using (fat.Pin())
                        {
                            ...
                            else if (upgrade != null || fat is IMyRadioAntenna || fat is IMyLaserAntenna || remoteControl != null || fat is IMyShipToolBase || fat is IMyMedicalRoom || fat is IMyCameraBlock || flightAi != null || cockpit != null && !cockpit.EnableShipControl || (fat is IMyConveyorSorter && TCDefList.Contains(fat.BlockDefinition.Id.SubtypeId)))
                            {
